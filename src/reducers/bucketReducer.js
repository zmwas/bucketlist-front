...
export default function bucketReducer(state = initalState.buckets, action) {
    switch (action.type) {
        case types.CREATE_BUCKET_SUCCESS:
            ...
            return Object.assign({}, state, {error: action.error, loading: action.loading, data: newState});
        ...
        case types.DELETE_BUCKET_SUCCESS: {
            ...
            const indexOfBucketToDelete = state.data.data.findIndex(bucket => bucket.id == action.bucket.id);
